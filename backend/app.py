...
def get_students():
    ...
    if not csv_file:
        print(f"[WARNING] CSV file not found. Tried paths: {CSV_PATHS}")
        return []
        ...
        with open(csv_file, "r", encoding="utf-8") as f:
            ...
            for idx, row in enumerate(reader, start=1):
                try:
                    ...
                    print(f"[WARNING] Skipping invalid row {idx}: {e}")
                    continue
        
        print(f"[SUCCESS] Loaded {len(students)} students from {csv_file}")
        return students
...
For system issues, contact: tech-support@institution.edu
        ...
        def send_guardian_email():
            try:
                ...
                print(f"[SUCCESS] Guardian email sent for Student ID: {data.student_id}")
            except Exception as e:
                print(f"[ERROR] Guardian email failed: {e}")
        
        def send_mentor_email():
            try:
                send_email(data.mentor_email, "⚠️ Student Alert - High Risk Detected", mentor_message)
                print(f"[SUCCESS] Mentor email sent for Student ID: {data.student_id}")
            except Exception as e:
                print(f"[ERROR] Mentor email failed: {e}")
        ...
        email_sent = True
        print(f"[INFO] Email sending initiated for Student ID: {data.student_id}")
